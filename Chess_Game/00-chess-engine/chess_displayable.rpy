...
init python:
    ...
    def quit_stockfish():
        """
        Cierra el motor Stockfish de forma segura y rápida.
        Usa close() que termina el proceso inmediatamente en lugar de quit()
        que espera una respuesta del motor. Esto evita que el juego se cuelgue.
        """
        if 'STOCKFISH_ENGINE' in global_objects and global_objects['STOCKFISH_ENGINE'] is not None:
            try:
                engine = global_objects['STOCKFISH_ENGINE']
                # Usar close() que cierra el proceso inmediatamente sin esperar respuesta
                # Esto evita que el juego se cuelgue si Stockfish no responde
                engine.close()
            except Exception as e:
                # Si hay error al cerrar, intentar forzar el cierre del proceso
                try:
                    # Si close() falla, intentar acceder directamente al transport
                    if hasattr(engine, 'transport') and engine.transport:
                        engine.transport.close()
                except:
                    pass
            finally:
                # Siempre limpiar la referencia, incluso si hubo errores
    ...
    class ChessDisplayable(renpy.Displayable):
        ...
        def __init__(self, fen=STARTING_FEN, player_color=None, depth=10):
            ...
            if self.player_color is None: # player vs player
                ...
                # Inicializar el motor si no existe (útil cuando se carga un save state)
